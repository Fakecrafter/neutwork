...
proc run(nn: Network, inputs: seq[float64]): seq[float64] =
  var inputs: seq[float64] = inputs
  for layer in nn:
    for i in 0 ..< layer.inputSize:
      layer.output[i] = calcNeuron(layer.weights[i], inputs, layer.bias[i])
    inputs = layer.output
...
proc createRandomNetwork(amount: seq[int]): Network =
  ...
  for counter ,layerH in amount:
    ...
    newLayer.bias = newSeq[float64](layerH)
